...
/*
 * Triple DES decryption of input from FILE pointer `fp' using key `sKey'
 * and tables specified in file `tablefile'.
 */
void decrypt3(const char* sKey, const char* tablefile, FILE* fp)
{
    if (strlen(sKey) != 48) {
    ...
    /* reverse first and third key for EDE mode */
    reverse(keys[0], 16);
    reverse(keys[2], 16);
